...
class CleanerApp(ctk.CTk):
    def __init__(self):
        ...
        window_width = 750  # Slightly smaller to comfortably fit 720p monitors
        window_height = 480
        ...
        # Set the geometry with the new size and center position
        self.geometry(f"{window_width}x{window_height}+{center_x}+{center_y}")

        # Make the window resizable
        self.resizable(True, True)
        # Add a minimum size so users can't shrink it so much that the UI breaks
        ...
        self.tab_temp_btn = ctk.CTkButton(
            self.sidebar_frame,
            text="Temp Cleaner",
            command=lambda: self.select_frame("temp"),
        )
        self.tab_temp_btn.grid(row=1, column=0, padx=20, pady=10)

        self.tab_uninst_btn = ctk.CTkButton(
            self.sidebar_frame,
            text="Uninstaller",
            command=lambda: self.select_frame("uninst"),
        )
        self.tab_uninst_btn.grid(row=2, column=0, padx=20, pady=10)

        self.tab_browser_btn = ctk.CTkButton(
            self.sidebar_frame,
            text="Browser Cleaner",
            command=lambda: self.select_frame("browser"),
        )
    ...
    def select_frame(self, name):
        # Reset button colors
        self.tab_temp_btn.configure(
            fg_color=["#3B8ED0", "#1F6AA5"] if name == "temp" else "transparent"
        )
        self.tab_uninst_btn.configure(
            fg_color=["#3B8ED0", "#1F6AA5"] if name == "uninst" else "transparent"
        )
        self.tab_browser_btn.configure(
            fg_color=["#3B8ED0", "#1F6AA5"] if name == "browser" else "transparent"
        )
    ...
    def setup_temp_frame(self):
        ctk.CTkLabel(
            self.temp_frame,
            text="System Temp Cleaner",
            font=ctk.CTkFont(size=24, weight="bold"),
        ).pack(pady=(0, 15))

        self.t_log_box = ctk.CTkTextbox(
            self.temp_frame, width=550, height=250, font=ctk.CTkFont(size=13)
        )
        self.t_log_box.pack(pady=10)
        self.t_log_box.insert(
            "0.0",
        ...
        self.t_scan_btn = ctk.CTkButton(
            btn_frame,
            text="Scan Storage",
            command=self.start_t_scan,
            font=ctk.CTkFont(weight="bold"),
        )
        self.t_scan_btn.pack(side="left", padx=10)

        self.t_clean_btn = ctk.CTkButton(
            btn_frame,
            text="Clean Now",
            command=self.start_t_clean,
            state="disabled",
            fg_color="#A32424",
            hover_color="#7A1B1B",
            font=ctk.CTkFont(weight="bold"),
    ...
    def _t_finish_scan(self, log, mb):
        ...
        if mb > 0:
            self.t_clean_btn.configure(state="normal")
    ...
    def setup_uninst_frame(self):
        ...
        ctk.CTkLabel(
            self.uninst_frame,
            text="Warning: This triggers the official uninstaller. Admin rights may be required.",
            text_color="gray",
        ).pack(pady=(0, 10))
        self.prog_scroll = ctk.CTkScrollableFrame(
            self.uninst_frame, width=550, height=350
        )
    ...
    def load_programs(self):
        for widget in self.prog_scroll.winfo_children():
            widget.destroy()
        ...
        for prog in programs:
            ...
            ctk.CTkLabel(frame, text=prog["name"], anchor="w").pack(
            ...
            ctk.CTkButton(
                ...
                fg_color="#A32424",
                hover_color="#7A1B1B",
                command=lambda n=prog["name"], c=prog["cmd"]: self.uninstall_app(n, c),
    ...
    def setup_browser_frame(self):
        ...
        ctk.CTkLabel(
            self.browser_frame,
            text="CLOSE YOUR BROWSERS BEFORE CLEANING",
            text_color="#FFaa00",
            font=ctk.CTkFont(weight="bold"),
        ).pack(pady=(0, 15))
        ...
        ctk.CTkLabel(
            b_frame, text="Select Browsers:", font=ctk.CTkFont(weight="bold")
        ).pack(anchor="w", padx=10, pady=5)

        b_chk_frame = ctk.CTkFrame(b_frame, fg_color="transparent")
        b_chk_frame.pack(fill="x", padx=10, pady=5)

        ctk.CTkCheckBox(b_chk_frame, text="Chrome", variable=self.chk_chrome).grid(
            row=0, column=0, padx=10, pady=5, sticky="w"
        )
        ctk.CTkCheckBox(b_chk_frame, text="Edge", variable=self.chk_edge).grid(
            row=0, column=1, padx=10, pady=5, sticky="w"
        )
        ctk.CTkCheckBox(b_chk_frame, text="Firefox", variable=self.chk_firefox).grid(
            row=0, column=2, padx=10, pady=5, sticky="w"
        )
        ctk.CTkCheckBox(b_chk_frame, text="Brave", variable=self.chk_brave).grid(
            row=0, column=3, padx=10, pady=5, sticky="w"
        )
        ctk.CTkCheckBox(b_chk_frame, text="Opera", variable=self.chk_opera).grid(
        ...
        ctk.CTkLabel(
            d_frame, text="Select Data:", font=ctk.CTkFont(weight="bold")
        ).pack(anchor="w", padx=10, pady=5)
        ctk.CTkCheckBox(d_frame, text="Cache", variable=self.chk_cache).pack(
            side="left", padx=20, pady=5
        )
        ctk.CTkCheckBox(d_frame, text="Cookies", variable=self.chk_cookies).pack(
            side="left", padx=20, pady=5
        )
        ctk.CTkCheckBox(d_frame, text="History", variable=self.chk_history).pack(
            side="left", padx=20, pady=5
        )
        ...
        self.b_scan_btn = ctk.CTkButton(
            btn_frame,
            text="Scan Browsers",
            command=self.start_b_scan,
            font=ctk.CTkFont(weight="bold"),
        )
        self.b_scan_btn.pack(side="left", padx=10)

        self.b_clean_btn = ctk.CTkButton(
            btn_frame,
            text="Clean Browser Data",
            state="disabled",
            fg_color="#A32424",
            hover_color="#7A1B1B",
            command=self.start_b_clean,
            font=ctk.CTkFont(weight="bold"),
        )
                ...
                if folder == "Default" or folder.startswith("Profile"):
                    ...
                    if self.chk_cache.get():
                        targets.extend(
                            [
                                (
                                    f"{profile_name} Cache",
                                    os.path.join(profile_path, "Cache"),
                                ),
                                (
                                    f"{profile_name} Code Cache",
                                    os.path.join(profile_path, "Code Cache"),
                                ),
                                (
                                    f"{profile_name} Cache Data",
                                    os.path.join(profile_path, r"Cache\Cache_Data"),
                                ),
                            ]
                        )
                    if self.chk_cookies.get():
                        targets.append(
                            (
                                ...
                                os.path.join(profile_path, r"Network\Cookies"),
                            )
                        )
                    if self.chk_history.get():
                        targets.append(
                            (
                                f"{profile_name} History",
    ...
    def get_browser_targets(self):
        ...
        if self.chk_chrome.get() and local_app:
            targets.extend(
                self.get_chromium_targets(
                    "Chrome", os.path.join(local_app, r"Google\Chrome\User Data")
                )
            )
        ...
        if self.chk_edge.get() and local_app:
            targets.extend(
                self.get_chromium_targets(
                    "Edge", os.path.join(local_app, r"Microsoft\Edge\User Data")
                )
            )
        ...
        if self.chk_brave.get() and local_app:
            targets.extend(
                self.get_chromium_targets(
                    "Brave",
                    os.path.join(local_app, r"BraveSoftware\Brave-Browser\User Data"),
                )
            )

        if self.chk_opera.get():
            for op_type, folder in [
                ("Opera", "Opera Stable"),
                ...
                if roaming_app and local_app:
                    ...
                    if os.path.exists(base_roam) or os.path.exists(base_loc):
                        if self.chk_cache.get():
                            targets.extend(
                                [(f"{op_type} Cache", os.path.join(base_loc, "Cache"))]
                            )
                        if self.chk_cookies.get():
                            targets.append(
                                [
                                    (
                                        f"{op_type} Cookies",
                                        os.path.join(base_roam, r"Network\Cookies"),
                                    )
                                ]
                            )
                        if self.chk_history.get():
                            targets.append(
                                [
                                    (
                                        f"{op_type} History",
                                        os.path.join(base_roam, "History"),
                                    )
                                ]
                            )
                    ...
                    for profile in os.listdir(base):
                        if self.chk_cache.get() and os.path.exists(local_base):
                            targets.append(
                                (
                                    "Firefox Cache",
                                    os.path.join(local_base, profile, "cache2"),
                                )
                            )
                        if self.chk_cookies.get():
                            targets.append(
                                (
                                    "Firefox Cookies",
                                    os.path.join(base, profile, "cookies.sqlite"),
                                )
                            )
                        if self.chk_history.get():
                            targets.append(
                                (
                                    "Firefox History",
                                    os.path.join(base, profile, "places.sqlite"),
                                )
                            )
    ...
    def _finish_b_scan(self, msg, total_mb):
        ...
        if total_mb > 0:
            self.b_clean_btn.configure(state="normal")
